...
func TestInitializeReturnsInstructions(t *testing.T) {
	...
	for _, want := range []string{"exec_run", "ssh", "awaiting_confirmation", "self-approve", "capabilities()", "asserted client id", "transport token", "remote process may continue", "not a session_id", "mission_create", "real successful job_id evidence"} {
