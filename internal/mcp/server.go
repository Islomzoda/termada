...
- For an operational task with a concrete outcome, start with mission_create(goal, plan). Use its dedicated session_id for all commands, update steps with their real successful job_id evidence, then finish with mission_update and export mission_report. After a daemon restart, mission_resume creates a fresh session attempt while preserving prior evidence.
